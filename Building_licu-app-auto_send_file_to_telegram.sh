...
# Так же, завершение скрипта командой kill или killall работает не очень корректно. Сам скрипт завершается, но при этом процесс inotifywait почему-то продолжает висеть в фоне.
